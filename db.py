...
async def wallet_bonus(tg_id: int, amount: int, op_id: str,
    ...
    Вейджер РАСТЁТ на amount×BONUS_WAGER_X — начисленное надо ОТЫГРАТЬ, прежде
    чем оно станет выводимым. Это касается и выигрышей фриспинов Kush Reels
    (kind='freespin'): победа на фриспине запирается бонусом и увеличивает
    отыгрыш, а НЕ падает сразу выводимыми деньгами — иначе бесплатные спины
    выкачивали бы кассу."""
    op_id = str(op_id or "").strip()
    if not op_id or len(op_id) > 64:
        raise ValueError("Неверный op_id")
