...
# кнопка на казино-мини-апп под постами. Чтобы кнопка СРАЗУ ЗАПУСКАЛА приложение
# (а не открывала чат), нужна app-ссылка t.me/<bot>/<app>. Короткое имя аппа
# казино — «play» (PROMO_CASINO_APP). Бота берём из CASINO_TG_LINK, добавляем /play.
_cas_base = os.getenv("CASINO_TG_LINK", "").split("?")[0].strip().rstrip("/")
_cas_app = os.getenv("PROMO_CASINO_APP", "play").strip("/")
if _cas_base:
    _parts = _cas_base.split("/")
    _cas_bot = "/".join(_parts[:4]) if len(_parts) >= 4 else _cas_base  # https://t.me/<bot>
    _promo_cas = f"{_cas_bot}/{_cas_app}" if _cas_app else _cas_bot
else:
    _promo_cas = ""
...
PROMO_TIMES = [t.strip() for t in os.getenv("PROMO_TIMES", "10:00,14:00,17:00,21:00,19:00").split(",") if t.strip()]
# слот PROMO_TIMES[i] публикует PROMO_POSTS[i] (пары по индексу, порядок ≠ хронология):
# 10:00 общий · 14:00 egrow-4 · 17:00 анонс викторины (за час до старта в 18:00) ·
# 21:00 реферальный · 19:00 рулетка (розыгрыш)
PROMO_POSTS = [
    ("promo/egrow-1.png", ""),
    ("promo/egrow-4.png", ""),
    ("promo/egrow-quiz.jpg",
     ...
     "• <b>10 вопросов</b> по теме-победителю\n"
     "• Вопрос показываем заранее — успеешь прочитать, потом варианты\n"
     "• Первый верный ответ забирает приз\n\n"
    ...
    ("promo/egrow-3.jpg",
     ...
     "🏁 <b>Новинка — гонка недели:</b> если за неделю все вместе приведут "
     "<b>20+</b> друзей — тот, кто пригласил больше всех, забирает <b>1000 ₴</b>!\n\n"
     "Твоя персональная ссылка — <b>прямо в Профиле приложения</b>."),
    ("promo/roulette.png",
     "🎰 <b>РУЛЕТКА Magic Market</b> — розыгрыш призов!\n\n"
     "Приведи <b>3 друзей</b> — забери гарантированный бонус и поборись за крупные "
     "призы на баланс.\n\n"
...
# сигнатуры разрешённых форматов: заявленный тип должен совпадать с содержимым
_RECEIPT_TYPES = {
...
def _receipt_ok(receipt: str) -> bool:
    """Квитанция: только jpeg/png/webp/pdf, и байты должны совпадать с заявленным типом.

    Проверки одного префикса мало: под «data:image/...» подсовывается что угодно —
    от svg со скриптом до кавычки, ломающей вёрстку в панели администратора."""
    if not isinstance(receipt, str) or len(receipt) > MAX_RECEIPT_LEN:
        return False
    m = re.match(r"^data:([\w.+-]+/[\w.+-]+);base64,([A-Za-z0-9+/=\s]+)$", receipt)
    if not m:
        return False
    mime = m.group(1).lower()
    sigs = _RECEIPT_TYPES.get(mime)
    if not sigs:
        return False
    ...
    if not any(head.startswith(s) for s in sigs):
        return False
    if mime == "image/webp" and head[8:12] != b"WEBP":   # RIFF бывает не только у webp
        return False
    return True
