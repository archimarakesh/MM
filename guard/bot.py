"""Magic Market — Guard-бот: капча-ознакомление с правилами чата.

Отдельный сервис (отдельный токен), не нагружает магазинного бота.
Новый участник → мьют + правила с кнопкой «Ознакомлен». Не нажал за N минут → кик.

ENV:
  GUARD_BOT_TOKEN   — токен отдельного бота (BotFather), обязателен
...
Бот должен быть админом чата с правами: банить и ограничивать участников, удалять сообщения.
"""
import asyncio
import html
import logging
...
TOKEN = os.getenv("GUARD_BOT_TOKEN", "")
...
async def gate_user(chat_id: int, u):
    ...
    try:
        m = await bot.send_message(
            chat_id,
            f"👋 <b>{esc(u.full_name)}</b>, добро пожаловать!\n\n{RULES_TEXT}\n\n"
...
async def main():
    if not TOKEN:
        raise SystemExit("GUARD_BOT_TOKEN не задан")
